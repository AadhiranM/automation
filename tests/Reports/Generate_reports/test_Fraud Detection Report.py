...
class Test_R_fraud_detection_report(BaseTest):
    ...
    def test_fraud_detection_report(self, driver, data):
        ...
        if toast_text and "Report generation has been initiated successfully!" in toast_text:
            self.logger.info("Fraud detection report successful")

        else:
            take_screenshot(
                driver,
                test_name="test_Fraud_detection_report_failed",
                folder_name="Screenshots\\reports\\Generate_reports\\Fraud detection_Report"
            )

            self.logger.error("Fraud detection report failed")
            assert False, "Fraud detection report generation failed"
        time.sleep(1)
